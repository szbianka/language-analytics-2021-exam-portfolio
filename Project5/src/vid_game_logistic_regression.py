...
def main():
    ...
    vid_game = pd.read_csv(data_path)

    genre = vid_game['Genre'].values
    platform = vid_game['Platform'].values

    # train and test split using sklearn
    X_train, X_test, y_train, y_test = train_test_split(platform, 
                                                        genre, 
                                                        test_size=0.2, 
                                                        random_state=42)
    ...
    outpath = os.path.join("..", "output", "vid_game_log-reg_model")
    file = open(outpath, "w")
    file.write(classifier_metrics)
    file.close()
    
    print(f"Done. Logistic Regression model classification report is saved into outputs folder.")
